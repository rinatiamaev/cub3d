...
/*   Updated: 2025/03/11 22:35:33 by nlouis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calculate_step_and_side_dist(t_los *los, t_dpoint src)
{
	...
	{
		los->step.x = -1;
		los->side_dist.x = (src.x - los->map_check.x) * los->delta_dist.x;
	}
	else
	{
		los->step.x = 1;
		los->side_dist.x = (los->map_check.x + 1.0 - src.x) * los->delta_dist.x;
	}

	if (los->ray_dir.y < 0)
	{
		los->step.y = -1;
		los->side_dist.y = (src.y - los->map_check.y) * los->delta_dist.y;
	}
	else
	{
		los->step.y = 1;
		los->side_dist.y = (los->map_check.y + 1.0 - src.y) * los->delta_dist.y;
	}
}
...
static bool	perform_los_dda(t_game *game, t_los *los, t_dpoint target)
{
	while (true)
	{
		...
		if (game->map->matrix[los->map_check.y][los->map_check.x] == WALL
			...
			return (false);
		if (los->map_check.x == (int)target.x && los->map_check.y == (int)target.y)
			return (true);
	}
}

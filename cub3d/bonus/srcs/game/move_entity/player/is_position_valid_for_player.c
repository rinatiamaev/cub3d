...
static bool	is_position_near_any_npc(t_game *game, t_dpoint next_pos)
{
	t_npc	*npc;
	double	npc_distance;
...
static bool	is_position_near_any_item(t_game *game, t_dpoint next_pos)
{
	t_item	*item;
...
bool	is_position_valid_for_player(t_game *game, t_dpoint next_pos)
{
	t_point	grid_pos;
	...
	if (!is_map_position_valid_player(game, next_pos))
		return (false);
	if (is_position_near_any_npc(game, next_pos))
		return (false);
	if (is_position_near_any_item(game, next_pos))
		return (false);

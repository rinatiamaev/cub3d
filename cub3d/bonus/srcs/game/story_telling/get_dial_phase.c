...
/*   Updated: 2025/03/19 13:58:06 by nlouis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_dial_phase	get_witch_kitty_phase(t_story_state *story)
{
	if (!story->has_spoken_to_witch)
		...
		return (PHASE_4);
	else if (story->sibling == SAVED && (story->key != FOUND && story->exit != FOUND))
		return (PHASE_5);
	else if (story->sibling == SAVED && (story->key == FOUND && story->exit == FOUND))
		return (PHASE_6);
	return (IDLE);
...
t_dial_phase	get_fire_spirit_phase(t_story_state *story)
{
	if (!story->has_spoken_to_fire_spirit)
		return (PHASE_0);
	else if (story->fireball == UNLOCKED || (story->key == FOUND && story->exit == FOUND))
		return (PHASE_1);
	return (IDLE);
}
